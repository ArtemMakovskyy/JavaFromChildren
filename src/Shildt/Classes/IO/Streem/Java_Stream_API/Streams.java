...
//https://www.youtube.com/watch?v=RzEiCguFZiY&t=27s
...
//    @Test
    public void creation() throws IOException {
        ...
        Stream<Integer> iterate = Stream.iterate(1950, val -> val + 3); // итератор с шагом 3

        Stream<String> concat = Stream.concat(stringStream, build); // складывает стримы
    }

//    @Test
    public void terminate() {
        Stream<Employee> stream = emps.stream();
        stream.count(); // количество объектов

        emps.stream().forEach(employee -> System.out.println(employee.getAge()));
        emps.forEach(employee -> System.out.println(employee.getAge()));//чтоб не дублировать стрисм запускаем у коллекции

        emps.stream().forEachOrdered(employee -> System.out.println(employee.getAge()));// дает приемущество в паралел стрим, что обойдет все е лементы

        emps.stream().collect(Collectors.toList());
        emps.stream().toArray();

        Map<Integer, String> collect = emps.stream().collect(Collectors.toMap(
                // преобразовываем первое поле в ключи, второе ва и мя и фамилию
                Employee::getId, //emps -> emps.getId();
                emp -> String.format("%s %s", emp.getLastName(), emp.getFirstName())
        ));

        IntStream intStream = IntStream.of(100, 200, 300, 400);
        int d = intStream.reduce((left, right) -> left + right).getAsInt();
...
//    @Test
    public void transform() {
        ...
        emps.stream()
                .skip(3)//біло 16, стало 13 -Ю затем обрабатываем 13.
                .limit(5);// из 13 берем первые 5 элементов

        emps.stream()
                .sorted(Comparator.comparingInt(Employee::getAge))
                .peek(emp -> emp.setAge(18))//устанавливает всем возвраст 18
...
//===========
        emps.stream().takeWhile(employee -> employee.getAge() > 30).forEach(System.out::println);//takeWhile отсекает все елементы, которые не соответствуют условию.
        System.out.println();
        emps.stream().dropWhile(employee -> employee.getAge() > 30).forEach(System.out::println);//dropWhile оставляет все елементы, которые соответствуют условию.
